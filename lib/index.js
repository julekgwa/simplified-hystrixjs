 ...
 */
 function createServiceCommand(func, service, functionName) {
 ...
 */
function createHystrixCommands(fn, service, circuitBreaker = 'opossum') {
  ...
  if (typeof fn === 'function') {
    ...
    serviceFns.forEach((func) => {
      hystrixFunctions[func] = createServiceCommand(fn[func], service, func);
    });
  } else {
...
function getPrometheusStream(library = 'opossum') {
  defaultCircuitBreaker = library;
  return defaultCircuitBreaker === 'opossum'
    ? circuitBreaker.stats
    : globalStats.getHystrixStream()
}

function createHystrixStream(app, path = '/manage/hystrix.stream') {
